...
def visualise_filled(df, start_date, end_date, FILE, events_dict):
    # Create figure with 3 subplots
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 12), sharex=True)
    fig.suptitle(f'Hive Analysis - {FILE}\n{start_date.strftime("%Y-%m-%d")} to {end_date.strftime("%Y-%m-%d")}', 
                fontsize=16)
    ...
    ax1.plot(df.index, df['t_i_4'], label='t_i_4')
    ax1.plot(df.index, df['t_i_5'], label='t_i_5')
    ax1.plot(df.index, df['temp_mid'], label='temp_mid')
    ax1.plot(df.index, df['t_o'], label='t_o')
    ax1.set_ylabel('Temperature (°C)')
    ...
    # Plot 2: Weight
    ax2.plot(df.index, df['weight_kg'], label='weight')
    ax2.plot(df.index, df['weight_kg_smoothed'], label='weight_kg_smoothed')

    ax2.set_ylabel('Weight (kg)')
    ax2.legend()
    ax2.grid(True)

    # Plot 3: weight_kg_3_pct
    ax3.plot(df.index, df['weight_kg_3_pct'], label='weight_kg_3_pct')
    ax3.set_ylabel('Weight 3 Percent Change')
    ax3.legend()
    ax3.grid(True)
